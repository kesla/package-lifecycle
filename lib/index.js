...
import {assign, getIn} from 'immutable-object-methods';
...
const runScript = async ({dir: cwd, script}) => {
  const path = await npmPath({cwd});
  const env = assign(process.env, {
    PATH: path
  });

  const [stdout, stderr] = await exec(script, {cwd, env});
